"""
OMOP Vocabulary Reader

Reads OMOP vocabulary tables from local CSV files (Athena download format).
Expects tab-separated files: CONCEPT.csv, CONCEPT_RELATIONSHIP.csv, CONCEPT_SYNONYM.csv,
and CONCEPT_SMALL.csv (generated by scripts/create_concept_small.py).
"""
...
class VocabularyCSVDataSource(BaseDataSource):
    """Data source for local OMOP vocabulary CSV files."""
    ...
    def __init__(
        ...
        """
        Initialize the vocabulary CSV data source.
        ...
        self.logger.info(f"Initialized vocabulary CSV data source: {data_folder}")
    ...
    def read_concept_small(
        ...
        if not self.concept_small_path.exists():
            self.logger.error(
                f"CONCEPT_SMALL file not found: {self.concept_small_path}\n"
                f"Run scripts/create_concept_small.py first to generate the file."
            )
            return
