...
Supports both OpenAI API and local sLLM.
"""
...
class MappingValidator:
    """Validates mapping results using LLM (OpenAI or Local sLLM)."""
        ...
        if use_local_llm:
            ...
            if HAS_LOCAL_LLM:
                try:
                    ...
                    logger.info(f"MappingValidator initialized (Local LLM: {local_llm_model})")
            ...
            try:
                ...
                if api_key:
                    ...
                    logger.info(f"MappingValidator initialized (OpenAI: {openai_model})")
                else:
    ...
    def validate_mapping(
        ...
        if self.use_local_llm:
            if not self.local_llm:
                logger.warning("Local LLM not initialized, skipping validation")
                return True  # 검증 건너뛰기
        else:
            if not self.openai_client:
                logger.warning("OpenAI client not initialized, skipping validation")
                return True  # 검증 건너뛰기
        ...
        try:
            if self.use_local_llm:
                return self._validate_with_local_llm(entity_name, concept_id, concept_name, synonyms)
            else:
                return self._validate_with_llm(entity_name, concept_id, concept_name, synonyms)
        except Exception as e:
            logger.error(f"Validation failed: {e}")
            return True  # 오류 시 검증 통과로 처리
...
"""
    ...
    def _validate_with_local_llm(
        ...
        """Validate mapping using local sLLM."""
        prompt = self._create_prompt(entity_name, concept_id, concept_name, synonyms)
        system_prompt = (
        ...
        try:
            ...
            if response:
                ...
                logger.debug(f"Local LLM validation: {is_valid}, reason: {reasoning}")
                return bool(is_valid)
            else:
