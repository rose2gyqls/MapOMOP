...
- Local LLM: 로컬 sLLM (Qwen, TinyLlama, Hari 등)
...
class Stage3HybridScoring:
    ...
    def __init__(
        ...
        Args:
            ...
            scoring_mode: 'llm', 'local_llm', 'hybrid', or 'semantic_only'
            include_stage1_scores: Include scores in LLM prompt
            local_llm_model: 로컬 LLM 모델명 (qwen, tinyllama, hari)
            local_llm_path: 로컬 LLM 모델 경로
        """
        ...
        elif self.scoring_mode == "local_llm":
            if HAS_LOCAL_LLM:
                try:
                    ...
                    logger.info(f"Stage 3 initialized (Local LLM mode, model: {local_llm_model})")
                except Exception as e:
                    logger.error(f"Local LLM initialization failed: {e}")
            else:
                logger.error("Local LLM module not available")
    ...
    def calculate_hybrid_scores(
        ...
        if self.scoring_mode == "llm":
            ...
            return self._score_local_llm(entity_name, stage2_candidates, entity_embedding)
        elif self.scoring_mode == "hybrid":
    ...
    def _score_local_llm(
        self,
        entity_name: str,
        candidates: List[Dict[str, Any]],
        entity_embedding: Optional[Any]
    ) -> List[Dict[str, Any]]:
        """Score using local sLLM evaluation."""
        if not self.local_llm:
            logger.error("Local LLM not initialized")
            # Fallback to semantic scoring
            logger.info("Falling back to semantic scoring")
            return self._score_semantic(entity_name, candidates, entity_embedding)
        ...
        # Get Local LLM scores
        try:
            llm_scores = self._call_local_llm(entity_name, results)
            
            if not llm_scores:
                logger.warning("Local LLM scoring failed, falling back to semantic")
                return self._score_semantic(entity_name, candidates, entity_embedding)
            ...
            self._log_results("Local LLM", sorted_results)
            return sorted_results
            
        except Exception as e:
            logger.error(f"Local LLM scoring failed: {e}")
            return self._score_semantic(entity_name, candidates, entity_embedding)
    ...
    def _call_local_llm(
        ...
        """Call local sLLM for scoring."""
        prompt = self._build_prompt(entity_name, candidates)
        system_prompt = (
            "You are a medical terminology mapping expert. "
            "Select the best OMOP CDM concept for the given entity. "
            "Always respond with valid JSON only."
        )
        
        try:
            response = self.local_llm.generate_json(prompt, system_prompt)
            
            if response:
                return self._parse_llm_response_dict(response, candidates)
            else:
                logger.warning("Local LLM returned invalid JSON")
                return None
    ...
    def _parse_llm_response_dict(
        ...
        """Parse already-parsed LLM response dict."""
        result = {}
        ...
        try:
            if 'best_match' in parsed and 'rankings' in parsed:
                ...
                logger.info(f"Local LLM selected best match: {best_id}")
            ...
            logger.error(f"Response dict parsing failed: {e}")
            return {}
